...
use crate::utils::{self, PUBLIC_DIR, info};
/*Listen TCP */
pub fn run_server(){
    ...
    .get("/:f", Box::new(|request, _| {
        info(utils::Info::RUN, "require file", request.get_path().as_str());
        std::path::Path::new(format!("{}{}", PUBLIC_DIR, request.get_path()).as_str()).into()
    }))
    .get("/Post/:f", Box::new(|request, _| {
        let decoded = percent_decode(request.get_path().as_str().as_bytes()).decode_utf8().unwrap().to_string();
